...
local function InitAuctionator()
    if CLASSIC and Atr_GetAuctionPrice and Atr_CalcDisenchantPrice then -- Auctionator ClassicFix
        ...
        Auctionator_GetDisenchantPrice = function(itemLink, class, rarity)
            return Atr_CalcDisenchantPrice(class, rarity, ItemUpgradeInfo:GetUpgradedItemLevel(itemLink))
        end
    elseif VERSION >= 50400 and VERSION < 60000 then -- MoP Compatibility
        if Atr_GetAuctionPrice and Atr_CalcDisenchantPrice then
            ItemUpgradeInfo = LibStub('LibItemUpgradeInfo-1.0', true)
            Auctionator_GetMarketPrice = function(name, itemID)
                return Atr_GetAuctionPrice(name)
            end
            ...
            print("[KiwiFarm] Auctionator API not available for MoP.")
            Auctionator_GetMarketPrice = function() return nil end
            Auctionator_GetDisenchantPrice = function() return nil end
        end
    elseif Auctionator and Auctionator.API and Auctionator.API.v1 then -- Retail or Classic with modern Auctionator
...
local function LevelingReset()
	...
	--DEFAULT_CHAT_FRAME:AddMessage("PRICE: |cffFF0000" .. GetItemPrice(itemlink) .. "|r")	
...
do
	...
	function addon:COMBAT_LOG_EVENT_UNFILTERED(_, timestamp, eventType, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
...
addon:SetScript('OnEvent', function(self, event, ...)
	...
	--#d1
	if config.visible and (not config.farmZones or config.reloadUI) then
		addon:Show()
	else
		addon:Hide()
	end
...
do
			...
			if level then
				...
				for index=1,#menuList do
					...
					if item.hidden==nil or not item.hidden(item) then
						--#d1 fix FIXME
						item.parentValue = UIDROPDOWNMENU_MENU_VALUE
						...
						if type(item.checked)=='function' then
							item.checkedf = item.checked
						end
						if item.checkedf then -- support for functions instead of only booleans
							item.checked = item.checkedf(item, level, frame)
						end						
	...
	do
		local function checked(info)
			...
			--DevTools_Dump(info);
			--DEFAULT_CHAT_FRAME:AddMessage("info.value: |cffFF0000" .. info .. "|r")			
			--return false --#d1 FIXME
			--return config.priceByQuality[getMenuValue(info)][info.value]			
			return config.priceByQuality[info.parentValue][info.value]
		end
	...
	do
		...
		local function checked(info)			
			...
			return getItemPriceSource(info.parentValue, info.value)
		end
		local function set(info)
	...
	do
		...
		local function checked(info)
			--#d1 FIXME
			--local sound, groupKey = info.value, getMenuValue(info)
			local sound, groupKey = info.value, info.parentValue
